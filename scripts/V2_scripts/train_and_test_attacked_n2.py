...
def main():
	...
	Classes = {0: "regularTraffic", 1: "Scenario1     "}

	regular_traffic = np.loadtxt('sensor2_results/out_sensor2_regular_l2_n2.txt')
	scenario_3 = np.loadtxt('sensor2_results/out_sensor2_s1_l2_n2.txt')
	...
	for iteration in range(numberIterations):
		...
		trainFeatures_scenario3, oClass_scenario3=extractFeatures(scenario3_train,Class=1)
		trainFeatures=np.vstack((trainFeatures_regular, trainFeatures_scenario3))
		...
		trainFeatures_scenario3S, oClass_scenario3=extractFeaturesSilence(scenario3_train, Class=1, threshold=silence_threshold)
		trainFeaturesS=np.vstack((trainFeatures_regularS, trainFeatures_scenario3S))
		...
		trainFeatures_scenario3W, oClass_scenario3=extractFeaturesWavelet(scenario3_train,scales,Class=1)
		trainFeaturesW=np.vstack((trainFeatures_regularW, trainFeatures_scenario3W))
		trainFeaturesW = np.nan_to_num(trainFeaturesW)

		allTrainFeatures=np.hstack((trainFeatures,trainFeaturesS,trainFeaturesW))
		trainClass=np.vstack((oClass_rt, oClass_scenario3))


		# Extract features for test set
		testFeatures_regular, oClass_rt=extractFeatures(regular_test,Class=0)
		testFeatures_scenario3, oClass_scenario3=extractFeatures(scenario3_test,Class=1)
		testFeatures=np.vstack((testFeatures_regular, testFeatures_scenario3))
		...
		testFeatures_scenario3S, oClass_scenario3=extractFeaturesSilence(scenario3_test,Class=1, threshold=silence_threshold)
		testFeaturesS=np.vstack((testFeatures_regularS, testFeatures_scenario3S))
		...
		testFeatures_scenario3W, oClass_scenario3=extractFeaturesWavelet(scenario3_test,scales,Class=1)
		testFeaturesW=np.vstack((testFeatures_regularW, testFeatures_scenario3W))
		testFeaturesW = np.nan_to_num(testFeaturesW)

		allTestFeatures=np.hstack((testFeatures,testFeaturesS,testFeaturesW))
		oClass=np.vstack((oClass_rt, oClass_scenario3))
	...
	SVC_metrics = np.sum(SVC_metrics.reshape(numberIterations, 2, 2), axis=0)
	...
	RBF_metrics = np.sum(RBF_metrics.reshape(numberIterations, 2, 2), axis=0)
	...
	Poly_metrics = np.sum(Poly_metrics.reshape(numberIterations, 2, 2), axis=0)
	...
	SVC2_metrics = np.sum(SVC2_metrics.reshape(numberIterations, 2, 2), axis=0)
	...
	neural_network = np.sum(neural_network.reshape(numberIterations, 2, 2), axis=0)
